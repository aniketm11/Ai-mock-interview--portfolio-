...
const answer = z.object({
  ...
  voiceMetrics: z.object({
    pace: z.number().min(0).max(300), fillerCount: z.number().min(0),
    silenceSeconds: z.number().min(0), hesitationCount: z.number().min(0),
    confidence: z.number().min(0).max(100), fluency: z.number().min(0).max(100)
  })
});
...
app.use(cors({
  origin: (origin, callback) => {
    if (!origin || !env.APP_ORIGIN || origin === env.APP_ORIGIN) return callback(null, true);
...
const session = (res, token) => res.cookie('token', token, {
  httpOnly: true, sameSite: 'lax', secure: env.NODE_ENV === 'production',
  maxAge: 604800000, path: '/'
}).json({ ok: true });
